...
| Electron shell + typed preload bridge + IPC channels (§6.3) | Backend Architecture Team | **[SEQ]** (foundation) | ✅ |
| Secrets via OS keychain (ElevenLabs / LLM keys) | Backend Architecture Team | **[SEQ]** | ⬜ (Mac follow-up — not needed for ffmpeg/draft) |

**Acceptance:** a headless harness runs `produce --json` and asserts the event sequence (phases
bracketed start/done, per-segment counts, exactly one terminal `result`); the adapter spawns the
real CLI and parses the stream. ✅ met (`app/test/spawn-engine.test.ts` + the real-CLI run).
...
**Acceptance:** editing a `SLIDE:` block updates the preview within ~1s; an invalid frame field
shows an inline error and disables Produce; saving writes byte-identical markdown the CLI parses.
✅ met (verified in-browser + `app/test/serialize.test.ts` round-trip).
...
| Doctor/preflight panel gating Produce (green-checks) | Core Engine Team | **[PAR]** | ✅ |
| Load-to-Palmier button + live timeline readback chip (reuse default clean behavior) | Timeline Integration Team | **[PAR]** | ⬜ (Mac follow-up) |

**Acceptance:** producing the example lesson shows live per-agent progress and ends green
(verified end-to-end on Linux/ffmpeg). Load + readback against a live Palmier is the Mac follow-up.
...
**Acceptance:** Revise on one segment shows a diff and cannot apply a text edit without confirmation
(Hard Rule #5); Draft produces a parseable `script.md` and diffs it before applying.

### Phase E3.5 — Deliver + Hand off to Devin (engine ✅ built · Deliver UI ✅ built · Ask-Devin ⬜planned)

> The **engine/CLI side already exists and is tested** on `vibedevview`: `palmier export` /
> `publish` / `moments` / `attach` (the `sql`/`api`/`supabase` targets, double-gated). The **Deliver
> panel UI over them is now built** in the Studio app (this PR); the "Ask Devin" hand-off surface
> and the companion hgdw-lms `api` endpoint remain.

| Deliverable | Owner | Stream | Status |
| --- | --- | --- | --- |
| `export` / `publish` / `moments` / `attach` engine commands + `AttachTarget` (sql/api/supabase) + double-gate | Core Engine Team | **[SEQ]** (everything below depends on it) | ✅ **built** (#7) |
| Deliver panel (§4.6): staged export→publish→moments→attach with the dry-run preview + `Apply` gate | Frontend Integration Team | **[SEQ]** | ✅ **built** (this PR) |
...
| Deliver guardrail tests (attach never writes without target+apply+playbackId) | QA / Test Team | **[PAR]** | ✅ (`app/test/deliver-adapter.test.ts`) |

**Acceptance:** the Deliver panel runs export (ffprobe-green) → publish (dry-run) → attach *preview*
with **zero** writes and zero credentials; the GUI stays dry-run-only (a real `--target`+`--apply`
is left to the CLI/Devin). ✅ met for the dry-run path.
...
**Parallelism note:** E0 was the critical path — the `--json` event stream **[SEQ]** and the
adapter boundary **[SEQ]** unblocked everything. With them landed, the editor/preview (E1),
produce/load (E2), and deliver (E3.5 UI) tracks run largely **[PAR]** across the Frontend, Slides,
and Timeline teams.
...
Three intentional deviations from the original plan, all adopted to make the tool genuinely
...
3. **The Deliver panel is dry-run-only in the GUI.** It runs `export` (real, creds-free) then
   `publish`/`moments`/`attach` in their **safe-by-default** mode (dry-run, `sql` target) and shows
   the would-write SQL/bundle. A real publish (`--target mux`) or real attach (`--target api|supabase`
   `--apply`) is deliberately left to the CLI / "Ask Devin", so the double-gate can never be tripped
   from a stray click. The deliver commands emit a single JSON result object, which the adapter
   parses whole (they are not part of the NDJSON progress stream).
...
| --- | --- | --- |
| **M0 — Contract frozen** | E0 event stream + IPC + adapter | headless harness asserts the event sequence; adapter parses the real CLI stream |
| **M1 — Author experience** | E1 editor + preview + validation | can write a lesson and see every slide live, no terminal |
| **M2 — MVP (internal)** | E2 produce/status + doctor gate | a teammate produces the example lesson end-to-end in the GUI |
| **M3 — AI + Revise** | E3 Draft-with-AI + surgical Revise | one-segment fixes and AI drafts work with the diff gate enforced |
| **M3.5 — Deliver + Devin** | E3.5 Deliver panel (engine + UI built ✅) + Ask-Devin | export→publish→attach *preview* runs in the GUI with zero writes; Ask-Devin (local) drives the same CLI |
| **M4 — Shippable** | E4 notarized build + wizard + smoke | clean-Mac install → produce → load with zero terminal steps |

**MVP = M2.** Drafting and Revise (M3) are fast-follows; Deliver (M3.5) wraps the already-built
delivery engine in a dry-run-only GUI; Ask-Devin and "Generate in Palmier" remain post-MVP.
